...
func holding_update():
	...
	if item.item_data is Placeable:
		_can_place_item = world_build_handler.can_build_at_position(mouse_grid_position)
...
func use():
	var mouse_grid_position = world_build_handler._mouse_pos_to_grid_pos()
	var _can_place_item = false
	var _in_range = global_position.distance_to(world_build_handler._grid_pos_to_world_pos(mouse_grid_position)) < build_range
	if item.item_data is Placeable:
		_can_place_item = world_build_handler.can_build_at_position(mouse_grid_position) and _in_range
		if _can_place_item:
			if world_build_handler.build_at_position(mouse_grid_position, item.item_data):
				item.decrease_stack(1)
	if item.item_data is Seedbag:
		pass
